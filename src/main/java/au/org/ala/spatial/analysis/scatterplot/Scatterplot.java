package au.org.ala.spatial.analysis.scatterplot;
...
public class Scatterplot {
    ...
    private void makeXYZdataset(double min1, double min2, double max1, double max2) {
        ...
        GrayPaintScale ramp = new GrayPaintScale(min, max);

        try {
            ramp = new GrayPaintScale(Math.log10(min + 1), Math.log10(max + 1));
        } catch (Exception e) {
            System.out.println("error producing GrayPaintScale for log10 of min,max: " + min + "," + max);
            e.printStackTrace();
        }
